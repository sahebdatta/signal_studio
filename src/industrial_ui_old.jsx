...
const sourcesApi = {
  ...
  browse: (payload) => api.post("/sources/browse-variables", payload),
};
...
// ═══════════════════════════════════════════════════════════════════════════
//  SOURCE MODAL  — connects to backend for test + browse, then saves via API
// ═══════════════════════════════════════════════════════════════════════════
...
function SourceModal({ groupId, existing, onClose, onSaved }) {
  ...
  const [variables, setVariables] = useState([]);         // from browse API
  const [selected, setSelected] = useState(existing?.variables?.map(v => v.tag) || []);
  ...
  const toggleVar = tag => setSelected(s => s.includes(tag) ? s.filter(x => x !== tag) : [...s, tag]);

  // ── Step 1: Test connection → POST /sources/test-connection
  const handleConnect = async () => {
    const payload = buildConnectionPayload();
    const result = await call(sourcesApi.test, payload);
    if (result?.success) setConnected(true);
  };

  // ── Step 2: Browse variables → POST /sources/browse-variables
  const handleBrowse = async () => {
    const payload = buildConnectionPayload();
    const result = await call(sourcesApi.browse, payload);
    if (result?.variables) {
  ...
  // ── Final save → POST or PUT /groups/:gid/sources
  const handleSave = async () => {
    const payload = {
      protocol: srcType,
      connection: buildConnectionPayload().connection,
      variables: variables
        .filter(v => selected.includes(v.tag))
        .map(v => ({
          tag: v.tag,
          name: v.name,
          alias: aliases[v.tag] || v.name,
          description: descs[v.tag] || "",
  ...
  // Pre-populate variable list in edit mode from existing data
  // const displayVars = variables.length
  //   ? variables
  //   : (existing?.variables || []).map(v => ({
  //     tag: v.tag, name: v.name, data_type: v.data_type || "Float", unit: v.unit || "",
  //     value: v.last_value ?? "—",
  //   }));

  const [displayVars, setNodes] = useState([]);
  ...
  return (
    <Modal title={isEdit ? "Edit Data Source" : "Add Data Source"} onClose={onClose} width={900}>
      ...
      {step === 0 && (
        <div className="fade-in">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 6 }}>
            {[["opcua", "OPC UA", "Industrial standard protocol"], ["modbus", "Modbus TCP", "Legacy PLC connectivity"],
            ["tcpip", "TCP/IP Raw", "Raw socket stream"], ["rest", "REST API", "HTTP polling source"]].map(([k, lbl, sub]) => (
      ...
      {step === 1 && (
        <div className="fade-in">
          ...
          {srcType === "tcpip" && <>
            ...
            <FF label="Data Format"><select value={extras.fmt || "json"} onChange={e => setX("fmt", e.target.value)}><option value="json">JSON</option><option value="csv">CSV</option><option value="raw">Raw Bytes</option></select></FF>
          </>}
          {srcType === "rest" && <>
          ...
          <div style={{ marginTop: 16, display: "flex", justifyContent: "space-between" }}>
            ...
            <button className="btn-primary" onClick={handleBrowse} disabled={!connected || loading}>
              {loading && <Spinner />}Browse Variables →
            </button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="fade-in">
          <div style={{ fontSize: 12, color: C.textMuted, marginBottom: 12 }}>
            Select variables — live values update via WebSocket
          </div>
          <div style={{ maxHeight: 300, overflow: "auto", border: `1px solid ${C.border}`, borderRadius: 8 }}>
            <TreeTable value={displayVars} selectionMode="checkbox" selectionKeys={selectedNodeKeys}
              onSelectionChange={(e) => setSelectedNodeKeys(e.value)} tableStyle={{ minWidth: '0rem' }}>
            ...
            {/* <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead>
                <tr style={{borderBottom:`1px solid ${C.border}`}}>
                  {["","Tag","Name","Type","Live Value"].map(h=>(
                    <th key={h} style={{padding:"8px 10px",textAlign:"left",color:C.textMuted,fontWeight:500,fontSize:11,background:C.surface}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {displayVars.map(t=>(
                  <tr key={t.tag} onClick={()=>toggleVar(t.tag)}
                    style={{borderBottom:`1px solid ${C.border}22`,cursor:"pointer",background:selected.includes(t.tag)?C.accentDim:"transparent"}}>
                    <td style={{padding:"8px 10px"}}><input type="checkbox" checked={selected.includes(t.tag)} onChange={()=>toggleVar(t.tag)} onClick={e=>e.stopPropagation()}/></td>
                    <td style={{padding:"8px 10px",fontSize:11,color:C.textMuted,fontFamily:"IBM Plex Mono,monospace"}}>{t.tag}</td>
                    <td style={{padding:"8px 10px",color:C.text,fontSize:12}}>{t.name}</td>
                    <td style={{padding:"8px 10px",fontSize:11,color:C.textMuted}}>{t.data_type}</td>
                    <td style={{padding:"8px 10px"}}>
                      <span style={{color:C.accent,fontFamily:"IBM Plex Mono,monospace",fontSize:12}}>
                        {liveValues[t.tag] ?? t.value ?? "—"}
                      </span>
                      {t.unit&&<span style={{color:C.textMuted,fontSize:11}}> {t.unit}</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div style={{ marginTop: 8, fontSize: 11, color: C.textMuted }}>{selected.length} variable(s) selected</div>
          <div style={{ marginTop: 16, display: "flex", justifyContent: "space-between" }}>
            <button className="btn-ghost" onClick={() => setStep(1)}>← Back</button>
            <button className="btn-primary" onClick={() => setStep(3)} disabled={!selected.length}>Set Aliases →</button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="fade-in">
          <div style={{ fontSize: 12, color: C.textMuted, marginBottom: 14 }}>Assign human-readable names and descriptions</div>
          <div style={{ maxHeight: 320, overflow: "auto" }}>
            {displayVars.filter(t => selected.includes(t.tag)).map(t => (
              <Card key={t.tag} style={{ marginBottom: 10, padding: "12px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontSize: 11, color: C.textMuted, fontFamily: "IBM Plex Mono,monospace", background: C.bg, border: `1px solid ${C.border}`, borderRadius: 4, padding: "2px 6px" }}>{t.tag}</span>
                  <span style={{ color: C.accent, fontFamily: "IBM Plex Mono,monospace", fontSize: 12 }}>{liveValues[t.tag] ?? t.value ?? "—"} {t.unit}</span>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div><label>Alias Name</label><input value={aliases[t.tag] || ""} onChange={e => setAliases(a => ({ ...a, [t.tag]: e.target.value }))} placeholder={t.name} /></div>
                  <div><label>Description</label><input value={descs[t.tag] || ""} onChange={e => setDescs(d => ({ ...d, [t.tag]: e.target.value }))} placeholder="Optional…" /></div>
                </div>
              </Card>
            ))}
          </div>
      ...
      {step === 4 && (
        <div className="fade-in">
          <div style={{ padding: "12px 14px", background: C.bg, borderRadius: 8, border: `1px solid ${C.border}`, marginBottom: 14 }}>
            {[["Protocol", srcType.toUpperCase()], ["Endpoint", addr], ["Variables", `${selected.length} selected`]].map(([k, v]) => (
          ...
          <div style={{ maxHeight: 200, overflow: "auto" }}>
            {displayVars.filter(t => selected.includes(t.tag)).map(t => (
              <div key={t.tag} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.border}22`, fontSize: 12 }}>
                <span style={{ color: C.textDim }}>{aliases[t.tag] || t.name}</span>
                <span style={{ color: C.textMuted, fontSize: 11 }}>{descs[t.tag] || "—"}</span>
              </div>
            ))}
          </div>
      ...
      <div style={{ minHeight: "100vh", background: C.bg, paddingBottom: 40 }}>
        <div style={{ borderBottom: `1px solid ${C.border}`, background: C.surface, padding: "0 24px", display: "flex", alignItems: "center" }}>
          <div style={{ padding: "14px 0", marginRight: 32 }}>
            <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: "-0.02em" }}>IIoT</span>
